...
class ImageGenerationAgent(BaseAgent):
    ...
    def __init__(self, model: str = "gpt-4o-mini", temperature: float = 0.3, image_model: str = "dall-e-3"):
    ...
    async def execute(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if task.get("batch", False) and "descriptions" in task:
            self.logger.info(f"Batch image generation requested for {len(task['descriptions'])} images")
            return await self._batch_generate_images(task["descriptions"], 
        ...
        image_path = await self.generate_image(description, caption, size, quality, style)
    ...
    async def generate_image(self, description: str, caption: str, 
                            size: str = "1792x1024", 
    ...
    async def _batch_generate_images(self, 
        ...
        tasks = [self.generate_image(desc, caption, size, quality, style) 
        ...
        self.logger.info(f"Batch generation completed: {len(successful_paths)} successful, {failed_count} failed")
