...
from ..models.report import ReportStructure, ReportSection
...
class DocumentStructureAgent(BaseAgent):
    ...
    async def execute(self, task: Dict[str, Any]) -> ReportStructure:
        ...
        structure_prompt = self._create_structure_prompt(topic, research, template)
        structure_response = await self._call_llm(STRUCTURE_SYSTEM_PROMPT, structure_prompt, response_format="json")
    ...
    def _create_structure_prompt(self, topic: str, research: List[Dict[str, Any]], template: Dict[str, Any]) -> str:
        ...
        for i, result in enumerate(research):
            ...
            if 'content' in result and result['content']:
                ...
                content_snippet = f" - {content[:100]}..." if len(content) > 100 else f" - {content}"
        ...
        return f"""Create a comprehensive, detailed structure for a professional report on: {topic}
...
REQUIREMENTS:
1. Create a highly detailed structure with major sections and multiple subsections
2. Each major section should support 1000-1500 words of in-depth content
3. Include sufficient subsections to support comprehensive coverage
...
8. Organize the structure to maximize depth and detail

INSTRUCTIONS:
- For each major section, include 3-5 subsections to allow for detailed content
- Ensure sections are organized logically with a clear narrative flow
- Maintain academic/professional tone in section titles
- Adapt the template as needed to fit the topic, adding sections where appropriate
- Design the structure to support comprehensive, in-depth treatment of the topic
...
Create the report structure now, focusing on comprehensiveness and depth:"""
        ...
        try:
            ...
            for line in structure_response.split("\n"):
                ...
                if not line.startswith("  "):  # Main section
                    ...
                    current_section.subsections.append(ReportSection(
                        title=line.strip(),
                        content=""
                    ))
